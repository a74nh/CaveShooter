...
public class MovementTypeHomingMissile extends MovementType {
    ...
    public void OnEnemyConstruction(Enemy enemy, double xAdditionalSpeed, double yAdditionalSpeed) {
    	...
    	enemy.mDirectionDegrees = mInitialDirection;
    	enemy.mDirectionRadians = Math.toRadians(mInitialDirection);
    }
	...
	public boolean updatePhysics(Enemy enemy, long now, double elapsed, int canvasWidth, int canvasHeight) {
		...
		double xdist = - enemy.mX;
		double ydist = - enemy.mY;
		...
		if ( enemy.vy * xdist - enemy.vx * ydist > 0 )     
			enemy.mDirectionDegrees += mTurnSpeed * elapsed /1000.0;
		else 
			enemy.mDirectionDegrees -= mTurnSpeed * elapsed /1000.0;	
		
		enemy.mDirectionRadians = Math.toRadians(enemy.mDirectionDegrees);

		//Move
		
		enemy.vx = enemy.mSpeed * Math.sin(enemy.mDirectionRadians) * elapsed /1000.0;  
		enemy.vy = enemy.mSpeed * Math.cos(enemy.mDirectionRadians) * elapsed /1000.0;
		enemy.mX += enemy.vx;
		enemy.mY += enemy.vy;
