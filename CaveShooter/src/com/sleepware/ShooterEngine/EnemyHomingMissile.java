...
public class EnemyHomingMissile extends Enemy{
    ...
    public EnemyHomingMissile(ShooterEngineContext shooterEngineContext, long now, double x, double y, double xAdditionalSpeed, double yAdditionalSpeed, EntityType type, MovementTypeHomingMissile movementType) {
		
    	super(shooterEngineContext, now, x, y, type);
    	mMovementType = movementType;

        mSpeed = (double)type.getInitialSpeed();
        mDirectionDegrees = movementType.mInitialDirection;
        mDirectionRadians = Math.toRadians(mDirectionDegrees);
	}
	...
	public boolean updatePhysics(long now, double elapsed, int canvasWidth, int canvasHeight) {
		...
		if ( vy * xdist - vx * ydist > 0 )     
			mDirectionDegrees += mMovementType.mTurnSpeed * elapsed /1000.0;
		else 
			mDirectionDegrees -= mMovementType.mTurnSpeed * elapsed /1000.0;	
		...
		vx = mSpeed * Math.sin(mDirectionRadians) * elapsed /1000.0;  
		vy = mSpeed * Math.cos(mDirectionRadians) * elapsed /1000.0;
		mX += vx;
		mY += vy;
		...
		if(distanceMoved>=dist) {
			...
			mX = Global.mPlayer.mX;
			mY = Global.mPlayer.mY;
		}
		else {
			
			mX += ((Global.mPlayer.mX-mX) / dist * distanceMoved);
			mY += ((Global.mPlayer.mY-mY) / dist * distanceMoved);
		}
    	*/
